...
- **批量下载自动化**: 自动批量下载多个飞书云文档
  - 支持手动输入 URL（每行一个）或上传文件（.txt/.csv）
  - 自动递归发现知识库子页面，**优化的单次遍历下载**
  - 自动标签页管理 - 打开、下载、关闭标签页
  - 实时进度跟踪和详细日志
  - 支持暂停/继续/停止控制
  - 文件直接保存到下载文件夹（无需"另存为"对话框）
  - **可配置的文件组织方式**（可在 选项 → 下载 中设置）：
    - **子文件夹组织**：创建带时间戳的文件夹（例如 `wiki-export-2026-02-27-143022/`）
    - **嵌套文件夹**：按照知识库的父子关系创建文件夹层级
  - **知识库清单文件**: 为知识库导出自动生成的组织文件
    - `wiki-manifest.md`: 表格索引 + 树状结构可视化，包含元数据
    - `wiki-manifest.csv`: 电子表格友好格式，便于筛选和排序
    - 显示页面层级、文件名、URL 和导出统计信息
    - 文件使用知识库中经过处理的页面标题命名

  ![批量下载](images/BatchDownload.png)

  **启用嵌套文件夹后的文件夹结构示例：**
  ```
  Downloads/
  └── wiki-export-2026-02-27-143022/
      ├── 主 wiki 页面.md
      ├── 组件/
      │   ├── 按钮.md
      │   └── 输入框.md
